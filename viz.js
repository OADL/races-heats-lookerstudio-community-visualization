...
const drawViz = (data) => {
  console.log('Incoming data:', data); // Debug line
  ...
  const rows = data.tables.DEFAULT.map(row => ({
    ...
    heat: row.heat ? row.heat[0] : '',
    lane: row.lane ? row.lane[0] : '',
    name: row.name ? row.name[0] : '',
    agegroup: row.agegroup ? row.agegroup[0] : '',
    academy: row.academy ? row.academy[0] : '',
  ...
  app.id = 'app';
  app.style.maxWidth = '601px';
  app.style.maxHeight = '1061px';
  app.style.minWidth = '601px';
  app.style.minHeight = '1061px';
  app.style.width = '601px';
  app.style.height = '1061px';
  app.style.boxSizing = 'border-box';
  app.style.padding = '0px';
  app.style.display = 'flex';
  app.style.flexDirection = 'column';

  // Race title (fixed at top)
  const raceTitle = document.createElement('div');
  raceTitle.className = 'main-title';
  raceTitle.textContent = `${raceName}`;
  raceTitle.style.flex = '0 0 auto';
  app.appendChild(raceTitle);

  // Scrollable heats container
  const heatsScroll = document.createElement('div');
  heatsScroll.style.flex = '1 1 auto';
  heatsScroll.style.overflowY = 'auto';
  heatsScroll.style.overflowX = 'hidden';
  heatsScroll.style.height = '100%';
  heatsScroll.style.width = '100%';
  heatsScroll.style.display = 'flex';
  heatsScroll.style.flexDirection = 'column';

  // For each heat
  Object.entries(heats).forEach(([heatName, swimmers]) => {
    // Sort swimmers by lane ascendingly
    swimmers.sort((a, b) => Number(a.lane) - Number(b.lane));
    if(raceName.includes('800m')) {
      swimmers.sort((a, b) => {
        if(a.lane === b.lane) {
    ...
    // Use a minimal container for each heat
    const heatDiv = document.createElement('div');
    heatDiv.className = 'heat-container';

    // Heat title (use smaller heading)
    ...
    if (raceName.includes('800m')) {
      thead.innerHTML = `<tr><th class='align-center col-small'>Lane</th><th class='align-center col-small'>Side</th><th class='align-left col-wide'>Name</th><th class='align-center'>Age Group</th><th class='align-center'>Academy</th></tr>`;
    } else {
      thead.innerHTML = `<tr><th class='align-center col-small'>Lane</th><th class='align-left col-wide'>Name</th><th class='align-center'>Age Group</th><th class='align-center'>Academy</th></tr>`;
    }
    ...
    swimmers.forEach(swimmer => {
      ...
      if (raceName.includes('800m')) {
        tr.innerHTML = `<td class='align-center col-small'>${swimmer.lane}</td><td class='align-center col-small'>${swimmer.side}</td><td class='align-left col-wide'>${swimmer.name}</td><td class='align-center'>${swimmer.agegroup}</td><td class='align-center'>${swimmer.academy}</td>`;
      } else {
        tr.innerHTML = `<td class='align-center col-small'>${swimmer.lane}</td><td class='align-left col-wide'>${swimmer.name}</td><td class='align-center'>${swimmer.agegroup}</td><td class='align-center'>${swimmer.academy}</td>`;
      }
      tbody.appendChild(tr);
    });
    table.appendChild(tbody);
    heatDiv.appendChild(table);
    heatsScroll.appendChild(heatDiv);
  });

  app.appendChild(heatsScroll);
